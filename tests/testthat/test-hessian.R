...
test_that("Hessian calculation for logistic regression is correct", {
  ...
  H_numerical <- numerical_hessian(function(b) neg_log_likelihood_logistic(b, X_test, y_test), beta_test)
  ...
  cat("Hessian Absolute Error:\n")
  print(hessian_error)

  cat("Max absolute error:", max(hessian_error), "\n")
